...
def decodepacket(binpacket):
    ...
    if packettype==4: #literal container
        ...
        print('processed command',binpacket[:11+5*n],'literal',int(chunk,2),str(11+5*n),'bits read')
        if '1' in remainder:
            #stack.append(remainder)
            #print('more commands left to process after literal')
            decodepacket(remainder)
        ...
        if lentype==0:
            ...
            #print('shortpacket',shortpacket)
          
            #stack.append(shortpacket)
           
           
            print('processed command',binpacket[:22+nbits],'instruction',instruction,nbits,'bits',22+nbits,'bits read')
            ...
            if '1' in remainder:
                #print('more to read from operator len type 0')
                decodepacket(remainder)
        elif lentype==1:
            ...
            print('processed command',binpacket[:18],'instruction',instruction,npackets,'packets 18 bits read')
            f.write('processed command '+str(binpacket[:18])+' instruction '+str(instruction)+' '+str(npackets)+' packets 18 bits read\n')
            if '1' in remainder:
                #print('more to read from operator len type 1')
                decodepacket(remainder)
